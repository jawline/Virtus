...
const char* FREngine::TAG = "Font Render Engine";
...
bool FREngine::Init() {
	...
	if (GRenderer::getRenderer()->getRenderLog() != 0)
		GRenderer::getRenderer()->getRenderLog()->writeLine(TAG,
				"Initialized FreeType font engine\n");
...
bool FREngine::Load(File* filenode) {
	...
	int Return = FT_New_Memory_Face(Library, data, filenode->length(), 0,
			&FontFace);
...
void FREngine::LoadFontmap(int PointSize) {
	...
	for (int c = 0; c < 255; c++) {
		...
		uint8_t* Data = new uint8_t[2 * newFontMap.CharList[c].Width
				* newFontMap.CharList[c].Height];

		for (int j = 0; j < newFontMap.CharList[c].Height; j++) {

			for (int i = 0; i < newFontMap.CharList[c].Width; i++) {

				Data[2 * (i + j * newFontMap.CharList[c].Width)] = Data[2 * (i
						+ j * newFontMap.CharList[c].Width) + 1] = (i
						>= GlyphSlot->bitmap.width || j
						>= GlyphSlot->bitmap.rows) ? 0
						: GlyphSlot->bitmap.buffer[i + GlyphSlot->bitmap.width
								* j];
			}
		}

		newFontMap.CharList[c].charTexture
				= GRenderer::getRenderer()->getGraphicsAPI()->newTexture();

		newFontMap.CharList[c].charTexture->loadTextureData(
				input_data_byte_luminance_alpha, newFontMap.CharList[c].Width,
				newFontMap.CharList[c].Height, false, Data);
...
void FREngine::Render(int PointSize, unsigned int X, unsigned int Y,
		std::string Text) {
...
void FREngine::Render(int PointSize, unsigned int X, unsigned int Y,
		unsigned int width, unsigned int height, std::string Text) {
	...
	for (unsigned int i = 0; i < Text.size(); i++) {
		...
		FontMaps[Index].Render(Text[i], X, Y, tempChar->OffsetX * XScaler,
				-tempChar->OffsetY * YScaler, tempChar->Width * XScaler,
				tempChar->Height * YScaler);
