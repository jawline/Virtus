...
BeeHive::BeeHive(unsigned int numberOfBees) {
	...
	stringstream nmBeeStrm;
	nmBeeStrm << numberOfBees;

	GEngine::getInstance()->getLog()->writeLine(TAG,
			string("BeeHive created with ") + nmBeeStrm.str() + " Bees");
}

BeeHive::~BeeHive() {
	//TODO: KILL BEES FIRST
	GEngine::getInstance()->getLog()->writeLine(TAG,
			"WARNING: HIVE isn't able to kill bees yet. get on it");
	...
	GEngine::getInstance()->getLog()->writeLine(TAG, "HIVE destroyed");
